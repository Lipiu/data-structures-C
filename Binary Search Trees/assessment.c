...
//count the nodes having the balance less than average balance in a BST
void average(BSTNode* node, float sum, unsigned int elements){
    if(node != NULL){
        average(node->left, sum, elements);
        sum += node->data.balance;
        average(node->right, sum, elements);
    }
}

BSTNode* count_nodes_balance(BSTNode* node, float average_balance, unsigned int* less_than_average_counter){
    if(node != NULL){
        average(node->left, average_balance, *less_than_average_counter);
        if(node->data.balance < average_balance){
            *less_than_average_counter += 1;
        }
        else{
            average(node->right, average_balance, *less_than_average_counter);
        }
    }
}
...
int main()
{
    ...
    //count nodes having balance less than average
    float avg = 60.4;
    int* counter = 0;
    float sum = 0;
    average(root, sum, 5);
    root = count_nodes_balance(root, avg, counter);
    printf("\nNumber of nodes with less than average balance: %u\n", counter);
